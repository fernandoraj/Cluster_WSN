...
//	private double metaThreshold = 0.5;
	...
	/**
	 * Array 2D (agrupamentos) a partir de sensores (Mensagens de sensores = WsnMsgResponse) <p>
...
//							if (numMessagesExpectedReceived >= expectedNumberOfSensors) // If all messagesResponse (from all nodes in Cluster to be splited) already done received
	...
	/**
	 * Test if all nodes in line number "row" of "tempCluster" are in "blkList", ie, if the sink have already received message responses from all nodes in the respective Cluster <p>
	 * [Eng] Testa se todos os nós em número de linha "row" do "tempCluster" estão dentro do "blkList", se o "sumidouro" já recebeu respostas de mensagens de todos os nós do respectivo Cluster
	 * @param tempCluster Novo GroupCluster com os nós para serem "Juntados" <p>[Eng] New GroupCluster with the nodes to be "merged" <b> tempCluster </b>
	 * @param row Linha do Novo GroupCluster a ser testada <p>[Eng] Line from the New GroupCluster to be tested <b> row </b>
	 * @param blkList Lista Negra com os nós que já receberam do sink <p>[Eng] Black List with the nodes that have already received by the sink <b> blkList </b>
	 * @return Verdadeiro se todos os nós em linha "linha" de "tempCluster" estão na "blkList" <p>[Eng] True if all nodes in line number "row" of "tempCluster" are in "blkList"
	 */
	 ...
	 * @param cluster Grupo de mensagens (que representam agrupamentos) que serão classificados de acordo com parâmetros ("energia residual 'e'saltos para o sink')<p>[Eng] Group of messages (representing clusters) which will be classified according with params ('residual energy' and 'hops to sink') <b> cluster </b>
	 */
	 ...
	 * 
	 * @param clusterGroup  Grupo de mensagens (representando agrupamentos) que têm os agrupamentos configurados <p>[Eng] Group of messages (representing clusters) to have the clusters configurated <b> clusterGroup </b>
	 */
	 ...
	 * @param lineMessages ArrayList de WsnMsgResponse <p>[Eng] ArrayList of WsnMsgResponse <b>lineMessages</b>
	 ...
	 * @param tempClusterReceiver Estrutura do cluster que receberá os sensores / cluster a partir da estrutura tempClusterGiver <p>[Eng] Cluster structure that will receive the sensors/clusters from the tempClusterGiver structure <b>tempClusterReceiver<\b>
	 * @param tempClusterGiver Estrutura do cluster que vai dar os sensores / cluster à estrutura tempClusterReceiver <p>[Eng] Cluster structure that will give the sensors/clusters to the tempClusterReceiver structure <b> tempClusterGiver </b>
	 * @param rowIndexes Índices de linha de "tempClusterGiver" para ser movido para "tempClusterReceiver" <p>[Eng] Line indexes from "tempClusterGiver" to be moved to "tempClusterReceiver" <b> rowIndexes </b>
	 * @param blackList Lista que os elementos serão removidos da <b>lista negra</b><p>[Eng] List which the elements will be removed from <b> blackList </b>
	 */
	 ...
	 * @param list Um objeto ArrayList (WsnMsgResponse) <p>[Eng] An ArrayList(WsnMsgResponse) object <b> list </b>
	 * @param msgToRemoveFromList Um objeto WsnMsgResponse <p>[Eng] A WsnMsgResponse object <b> msgToRemoveFromList </b>
	 */
	 ...
	 * @param lineFromCluster Número da linha do cluster para ser dividido <b> lineFromCluster </b><p>[Eng] Line number from cluster to be divided <b> lineFromCluster </b>
	 */
	 ...
	 * @param tempCluster Cluster (acertamento de cluster) para ser utilizado como base - A estrutura é realmente um acertamento de cluster, em que cada cluster é representado por uma linha diferente <p>[Eng] Cluster (set of clusters) to be used as base - The structure actually is a set of clusters, where each cluster is represented by a different line <b> tempCluster </b>
	 * @param lineFromCluster Número da linha do cluster atual <p>[Eng] Line number from current cluster <b> lineFromCluster </b>
...
*/
	 ...
	 * @param newWsnMsgResp MessageResponse representando o nó sensorial para ser localizado na messageGroups <p>[Eng] MessageResponse representing the sensor node to be localized in messageGroups <b> newWsnMsgResp </b>
	...
	/**
	 * Ele identifica qual agrupamento (número da linha), onde o "representativeNode" está <p>
	 * [Eng] It identifies which cluster (line number) where the "representativeNode" is 
	 * @param representativeNode Nó representando o nó sensor representante a ser localizado no messageGroups <p>[Eng] Node representing the representative sensor node to be localized in messageGroups <b> representativeNode </b>
	 * @return Número da linha (agrupamento) do nó passado, caso contrário, retorna -1 indicando que não existe tal nó ("representativeNode") em qualquer agrupamento <p>[Eng] Line number (cluster) from node passed by; otherwise, returns -1 indicating that there is no such node("representativeNode") in any cluster
	 */ 
	...
	/**
	 * Ele remove o nó (newWsnMsgResp.source passado por parâmetro ) do grupo / agrupamento indicado por "tempCluster" e indicar se o grupo tornou-se vazio <p>
	 ...
	 * @param tempCluster Grupo de aglomerados que o nó (newWsnMsgResp.source) que serão removidos <p>[Eng] Group of clusters which the node (newWsnMsgResp.source) will be removed <b> tempCluster </b>
	 * @param newWsnMsgResp  Mensagem que contém o nó (newWsnMsgResp.source), que será removido do grupo de aglomerado <p>[Eng]Message that contains the node (newWsnMsgResp.source) which will be removed from group of cluster <b> newWsnMsgResp </b>
	 * @return Se o agrupamento desta mensagem / nó ficou vazio após a remoção do nó "newWsnMsgResp.source" [Eng] If the cluster of this message / node became empty after removal of the node in "newWsnMsgResp.source"
	 */
...
*/
	 ...
	 * @param tempCluster cluster(ArrayList), que terá os sensores ordenados (linha por linha) pela energia residual máxima <p>[Eng] Cluster (ArrayList) which will have the sensors ordered (line by line) by the max residual energy <b> tempCluster </b>
	 */
	 ...
	 * @param tempCluster Cluster(ArrayList), que terá os sensores ordenados (linha por linha - como um segundo critério) pelo número mínimo de saltos para o sink <p>[Eng] Cluster (ArrayList) which will have the sensors ordered (line by line - as a second criterion) by the min number of hops to sink <b> tempCluster </b>
	 */
...
//					System.out.println("Line = "+line+", Col = "+col+": NodeID = "+currentWsnMsgResp.source.ID+" BatLevel = "+currentWsnMsgResp.batLevel+" Round = "+((SimpleNode)currentWsnMsgResp.source).lastRoundRead);
	...
	/**
	 * Muda a mensagem de posição "index" para a primeira posição [0] em que a linha do array<p>
	 ...
	 *  
	 * @param tempCluster ArrayList2d de sensores, organizados em clusters (linha por linha) <p>[Eng] ArrayList2d from sensors, organized as clusters (line by line) <b> tempCluster </b>
	 * @param newWsnMsgResp Mensagem a ser utilizada para classificar o nó sensor <p>[Eng] Message to be used for classify the sensor node <b> newWsnMsgResp </b>
	 */
	 ...
	 * @param currentWsnMsg Mensagem atual já classificada no cluster <p>[Eng] Current message already classified in cluster <b> currentWsnMsg </b>
	 * @param newWsnMsg Nova mensagem a ser classificada no cluster <p>[Eng] New message to be classified in the cluster<b> newWsnMsg </b>
...
*/
	 ...
	 * @param currentWsnMsg Representa a mensagem atual do grupo de mensagens (messageGroups) em um "<WsnMsgResponse> ArrayList2d" estrutura <p> [Eng] Represents the current message from the group of messages (messageGroups) in a "ArrayList2d<WsnMsgResponse>" structure <b> currentWsnMsg </b>
	 * @param newWsnMsg Representa a mensagem chegou recentemente no nó sorvedouro, enviado a partir do nó sensor fonte <p>[Eng] Represents the recently arrived message in the sink node, sent from the source sensor node <b> newWsnMsg </b>
...
*/	
	 ...
	 * 
	 * @param wsnMsgResp Mensagem recebida com os parâmetros a serem lidos <p>[Eng] Received message with the parameters to be read <b> wsnMsgResp </b>
	 * @param clusterHeadNode Indica(seta) o ClusterHead daquele cluster <p>[Eng] Sets that the cluster clusterhead  <b> clusterHeadNode </b>
	 */
...
//			tipos = wsnMsgResp.getDataRecordTypes();
	 ...
	 * @param values Array de valores reais de entrada <p>[Eng] Array of real values ??input <b> values </b>
	 ...
	 * @param mediaValores Média dos valores lidos pelos sensores <p>[Eng]Mean of values ??read by the sensors <b> mediaValores </b>
	 ...
	 * @param mediaValores Média dos valores lidos pelos sensores <p>[Eng]Mean of values ??read by the sensors <b> mediaValores </b>
	 * @param mediaTempos Média dos tempos de leitura dos valores pelos sensores <p>[Eng] Mean time reading the values ??from sensors <b> mediaTempos </b>
	 * @param B Valor do coeficiente B da equação de regressão <p>[Eng] Value of the coefficient B of the regression equation <b> B </b>
	 ...
	 * @param wsnMsgResp Mensagem de resposta enviada do nó de origem para o nó sink, que agora enviará os (novos) coeficientes calculados para o nó de origem <p>[Eng] Response message sent from the source node to the sink node, which now sends the (new) coefficients calculated for the source node <b> wsnMsgResp </b>
	 * @param coeficienteA Valor do coeficiente A da equação de regressão <p>[Eng] Value of the coefficient A of the regression equation <b> coeficienteA </b>
	 * @param coeficienteB Valor do coeficiente B da equação de regressão <p>[Eng] Value of the coefficient B of the regression equation <b> coeficienteB </b>
	 */
	 ...
	 * @param rn Nó representante <p> [Eng] Representative Node <b> rn </b>
